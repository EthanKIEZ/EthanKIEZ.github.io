...
| 概念 | 本质 | 作用 |
|------|------|------|
| **上下文（Context）** | 进程在某一时刻的完整“现场” | 让进程被切走后还能原样恢复运行 |
| **PCB** | 操作系统内核中的数据结构 | 持久保存进程上下文的“容器” |
| **上下文切换（Context Switch）** | 操作系统执行的一次操作 | 把当前 CPU 的上下文保存到 PCB，再从另一个 PCB 恢复到 CPU |

**用一个比喻理解**：

- 上下文就像你读到一半的书的“状态”：第几页、哪一行、做了哪些批注。
- PCB 就像你的“书签/笔记本”，把这一状态记下来。
- 上下文切换就像你放下 A 书、拿起 B 书之前，先把 A 的状态记到书签里，再从 B 的书签里恢复它上次读到的位置。
...
1. **硬件上下文（CPU 相关）**
   ...
   - **通用寄存器**：`eax`、`ebx`、`rsp`、`rbp` 等，保存计算中间结果
   - **程序状态字 PSW**：标志位、中断使能状态、运行态/内核态等
   - **栈指针 SP**：用户栈和内核栈的位置

2. **系统上下文（操作系统相关）**
   - **地址空间信息**：页表基址、代码段/数据段/堆/栈的范围
   - **打开文件表**：进程正在读写的文件描述符
   - **信号、调度信息**：优先级、剩余时间片、等待事件
   - **进程状态**：RUNNING / READY / BLOCKED

##### 为什么需要 PCB

CPU 同一时间只能保存一份上下文。如果操作系统要支持多个进程并发，就必须在进程不运行时把它的完整状态找个地方存起来。PCB 就是这个“存状态的地方”。

> **没有 PCB，CPU 一旦切去执行别的进程，原来进程的现场就丢失了，再也无法正确恢复。**

##### 上下文切换的具体步骤

以从进程 A 切换到进程 B 为例：

1. **触发切换**：时钟中断到期、进程 A 发起 I/O、进程 A 执行完等。
2. **保存 A 的硬件上下文**：把 PC、寄存器、PSW 等写入 A 的 PCB。
3. **更新 A 的状态**：通常从 RUNNING 改为 READY 或 BLOCKED。
4. **选择下一个进程**：调度器从就绪队列中选出进程 B。
5. **恢复 B 的硬件上下文**：从 B 的 PCB 中读出 PC、寄存器等，写回 CPU。
6. **切换地址空间**：更新页表/TLB，让 CPU 看到的是 B 的内存。
7. **跳转到 B 的 PC 处继续执行**：B 感觉自己“从未被中断过”。

> **关键点**：上下文切换本身也消耗 CPU 时间（保存/恢复寄存器、刷新 TLB 等）。切换太频繁会降低系统效率，所以真实系统会用时间片、优先级等手段权衡。
{: .notice--info}
...
### 为什么 `-s 1` 能让 `-l 3:50` 的“随机”结果固定下来

`process-run.py` 的 `-s` 参数用来设置**随机种子（random seed）**。它本身不改变进程调度的逻辑，只改变 `-l X:Y` 随机生成指令序列时调用的随机数序列。

#### 伪随机数发生器（PRNG）

Python 的 `random` 模块使用的是**伪随机数发生器**（Pseudo-Random Number Generator）。它不是真正“随机”的，而是从一个初始值（种子）出发，通过固定算法生成一长串看起来随机的数字。

关键特性：**相同的种子，永远产生相同的数字序列。**

在 `process-run.py` 里，相关代码只有两行：
...
调用 `-s 1` 时，就是把 `random` 的状态初始化到种子 `1` 对应的位置。之后每次 `random.random()` 都会按固定顺序取值。
...
```python
if random.random() < chance_cpu:
    self.proc_info[pid][PROC_CODE].append(DO_COMPUTE)
else:
    self.proc_info[pid][PROC_CODE].append(DO_IO)
```

`-l 3:50` 表示每个进程 3 条指令，`chance_cpu = 50 / 100 = 0.5`。所以每条指令：

- `random.random() < 0.5` → CPU
- `random.random() >= 0.5` → I/O

#### 种子 1、2、3 实际产生的指令序列

用上面的规则可以算出：

| 种子 | 第 1 条 | 第 2 条 | 第 3 条 |
|------|---------|---------|---------|
| `-s 1` | CPU（0.134 < 0.5） | I/O（0.847 ≥ 0.5） | I/O（0.764 ≥ 0.5） |
| `-s 2` | I/O（0.956 ≥ 0.5） | I/O（0.948 ≥ 0.5） | CPU（0.057 < 0.5） |
| `-s 3` | CPU（0.238 < 0.5） | I/O（0.544 ≥ 0.5） | CPU（0.370 < 0.5） |

因此 `-s 1 -l 3:50` 每次运行都会得到完全相同的指令序列：**CPU → I/O → I/O**。调度结果自然也就固定了（表格里 seed=1 的总时间都是 15）。

#### 为什么要设计 `-s` 这个参数

- **可复现性**：做实验或写报告时，别人用同样的命令、同样的种子，能得到完全一样的输出。
- **对比公平性**：改变 `-S`、`-I`、`-L` 等调度参数时，保持 `-s` 相同，才能确保差异来自调度策略本身，而不是随机生成的负载不同。
- **调试方便**：如果某次运行出现奇怪现象，用同样的种子可以反复复现。

> **一句话总结**：`-s` 不是让随机数“更随机”，而是让伪随机序列“可重复”。在 `-l 3:50` 这种依赖随机数生成负载的场景里，固定种子就等于固定了指令序列，从而固定了实验结果。
{: .notice--info}
