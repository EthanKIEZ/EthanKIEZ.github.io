...
{% capture thinking_content %}
你可以先自己思考下面的问题，再展开查看参考思路。

### 1. 给 `process-run.py` 加上时间片轮转（Round Robin）

**思路**

当前模拟器里进程会一直跑到发起 I/O 或结束为止。要支持 RR，需要引入**时间片（quantum）**：每个进程连续运行不能超过 `quantum` 个 tick，用完就要让出 CPU。

**大致改法**

1. 给每个进程增加一个计数器，记录已连续运行多少 tick。
2. 在 `run()` 主循环中，每执行完一条指令后检查：如果时间片用完且还有其他 READY 进程，调用 `next_proc()` 切换。
3. 进程主动发起 I/O 或结束时，仍然正常切换。

**会观察到的现象**

- CPU 密集型长任务不再霸占 CPU，多个进程会更公平地轮流执行。
- 短任务/交互任务的响应变快。
- 时间片越小，上下文切换越频繁，开销占比越大。

> **结论**：RR 在公平性和开销之间权衡。时间片太大接近 FIFO，太小则切换开销高，真实系统通常取几十毫秒。

---

### 2. Linux `task_struct` vs xv6 `struct proc`

xv6 的 `struct proc` 已经很精简，主要保存：内存、状态、文件、上下文切换信息。

Linux 的 `task_struct`（在 `include/linux/sched.h`）要复杂得多，因为它要支持多核、CFS/实时调度、信号、命名空间、cgroup、seccomp 等。

| 方面 | xv6 `struct proc` | Linux `task_struct` |
|------|-------------------|---------------------|
| 规模 | 十几个字段 | 几百个字段 |
| 调度 | 简单阻塞唤醒 | CFS、RT、Deadline 多调度类 |
| 内存 | `char *mem` 直接指向物理页 | `mm_struct` 完整虚拟内存 |
| 文件 | 固定数组 `ofile[]` | `files_struct` 动态文件表 |
| 隔离/安全 | 几乎没有 | namespace、cgroup、capability |

> **关系**：xv6 的 `struct proc` 可以看作 Linux `task_struct` 的极度简化版，核心思想一致——保存进程状态、地址空间、打开文件和调度上下文。

---

### 3. 多道程序设计为什么提升 CPU 利用率？

**核心原因：CPU 和 I/O 可以并行工作。**

没有多道程序设计时，进程发 I/O 后 CPU 只能空等；有了多道程序设计，进程 A 发 I/O 被阻塞，CPU 立刻切去执行进程 B，等 A 的 I/O 完成再切回来。

**与上下文切换开销的权衡**

| 进程数太少 | 进程数适中 | 进程数太多 |
|-----------|-----------|-----------|
| I/O 完成时 CPU 没事做 | CPU 和 I/O 都忙 | 切换太频繁，开销占主导 |
| CPU 利用率低 | 利用率最高 | 吞吐量反而下降 |

> **结论**：多道程序设计的收益来自 **CPU 与 I/O 重叠**；代价是 **上下文切换开销**。目标是让 CPU 和 I/O 都尽量忙，同时切换不要太频繁。

---

### 4. 真实 I/O 路径与 `process-run.py` 的对应

真实 `read()` 大致经历：

```text
用户态发起请求 → 陷入内核 → VFS/文件系统 → 设备驱动 → DMA 传输
    → 设备中断 → 中断处理唤醒进程 → 返回用户态
```

对应到模拟器：

| 真实步骤 | `process-run.py` 表示 |
|---------|----------------------|
| 用户态发起 I/O 请求 | `RUN:io`（1 tick） |
| 陷入内核、驱动设置、DMA 启动 | 隐含在 `io` 指令中 |
| I/O 设备实际工作 | `BLOCKED` 持续 `-L` 个 tick |
| 中断到达、I/O 完成 | 第 `clock_tick + L + 1` 个 tick |
| 进程恢复、处理完成 | `RUN:io_done`（1 tick） |
| 返回用户态继续执行 | `io_done` 之后的下一条指令 |

> **注意**：`io` / `io_done` 不是真实 CPU 指令，而是对「发起 I/O」和「完成处理」两个内核态事件的抽象；中间 `BLOCKED` 区间就是设备真正干活的时间，也是多道程序设计能利用的窗口。
{% endcapture %}

<details class="thinking-box">
<summary><strong>拓展思考（点击展开查看参考答案）</strong></summary>
{{ thinking_content | markdownify }}
</details>
