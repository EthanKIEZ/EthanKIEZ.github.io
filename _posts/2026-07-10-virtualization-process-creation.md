...
以 `execl("/bin/ls", "ls", "-l", "/bin/ls", NULL)` 为例：

| 参数 | 含义 |
|---|---|
| `"/bin/ls"` | 要加载执行的**程序文件路径** |
| `"ls"` | 传给新程序的 `argv[0]`，通常是程序名 |
| `"-l"` | 第一个真正参数，`ls` 的选项 |
| `"/bin/ls"` | 第二个真正参数，`ls` 要查看的目标文件 |
| `NULL` | 参数列表结束标志 |
...
所以 `exec()` 后面的 `perror()` 和 `exit(1)` 只有在 `exec()` 失败时才会执行。

#### 为什么可以拿 `ls` 当"输入"

`exec()` 的第一个参数不是被处理的数据，而是**要替换当前进程的新程序文件**。

执行 `exec("/bin/ls", ...)` 后：

1. 当前进程的代码、数据、堆栈被清掉
2. `/bin/ls` 的代码、数据被加载进来
3. CPU 从 `/bin/ls` 的 `main()` 开始执行
4. 进程 PID 不变，但里面运行的程序已经变成 `ls`

因此 `ls` 不是 `exec()` 的"数据输入"，而是 `exec()` 要"换装"成的目标程序。
