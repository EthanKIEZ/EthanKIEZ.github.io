---
  ...
  date: 2026-06-18
  tags: [C, 学习笔记, CMake, CLion]
  ---

  ## 前言

  今天开始系统学习 C 语言，通过 6 个实战项目从基础语法逐步深入到指针、链表和文件操作。记录一下学习过程中的关键知识点。

  ## 项目结构

  使用 CMake 统一管理所有项目，每个项目一个子目录：

  untitled2/ ├── CMakeLists.txt ├── project01_calculator/        # 基础计算器 ├── project02_guessing_game/     # 猜数字游戏 ├── project03_student_mana
  ger/   # 学生成绩管理 ├── project04_contact_book/      # 简易通讯录 ├── project05_linked_list/       # 链表实现 └── project06_todo_list/         #
  命令行 TODO 清单
  ...
  注意：scanf(" %c", &operator) 中 %c 前面的空格用于跳过之前输入留下的换行符。
  ...
  fscanf(file, ...) 从指定文件读取，scanf(...) 从键盘读取。加载联系人时显然要从文件读。
  ...
  今天从零开始搭建了 C 语言学习环境，完成了 6 个由浅入深的项目，理解了编译链接、头文件、指针等核心概念。下一步计划继续练习动态内存管理和更复杂的数据结
  构。
